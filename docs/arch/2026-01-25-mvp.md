...
`ccx` is a CLI tool for managing Claude Code sessions running inside tmux. It allows users to start background Claude sessions, monitor their status, and stop them when complete.
...
```bash
ccx start "explain this codebase"
